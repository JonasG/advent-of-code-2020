...
namespace advent_of_code_2020_c_
{
    class Day2
    {
        ...
        private static bool ValidPart1(string passwordString)
        {
            var parts = passwordString.Replace(":", string.Empty).Split(new char[] { '-', ' ' });
            (int lowerBound, int upperBound, char character, string password) = (int.Parse(parts[0]), int.Parse(parts[1]), parts[2][0], parts[3]);
            int matches = password.Count(x => x == character);
            return lowerBound <= matches && matches <= upperBound;
        }
        
        private static bool ValidPart2(string passwordString)
        {
            var parts = passwordString.Replace(":", string.Empty).Split(new char[] { '-', ' ' });
            (int firstIndex, int secondIndex, char character, string password) = (int.Parse(parts[0]) - 1, int.Parse(parts[1]) - 1, parts[2][0], parts[3]);
            return password[firstIndex] != password[secondIndex] && (password[firstIndex] == character || password[secondIndex] == character);
        }

        public static void Run()
        {
            Console.WriteLine($"Day 2 Part 1 answer: {puzzleInput.Where(ValidPart1).Count()}");
            Console.WriteLine($"Day 2 Part 2 answer: {puzzleInput.Where(ValidPart2).Count()}");
        }
    }
}